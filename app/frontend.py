...
st.set_page_config(page_title="🧠 ARTECH Role-Based Chatbot", layout="centered")
...
with st.sidebar:
    ...
    # If logged in, show user details
        ...
        st.markdown(f"**🧾 Role:** `{st.session_state.user['role']}` (ARTECH)")
...
if st.session_state.user:
    ...
    with st.expander("📘 ARTECH Role & Access Explanation", expanded=False):
        user_role = st.session_state.user["role"].lower()
        if "c-levelexecutives" in user_role:
            st.info("Unfiltered access — full visibility (Executives, ARTECH).")
        elif "employee" in user_role:
            st.info("Filtered access — only general category documents (Employee, ARTECH).")
        else:
